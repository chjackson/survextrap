...
A multinomial logistic distribution defines the prior for the coefficients $p_k$ that define the mass of the hazard in each region: $log(p_k / p_1) = b_k$, with $b_1=0$ and $b_k \sim Logistic(\mu_k, \sigma)$ for $k = 2, \ldots, K$.  This prior is defined by:
...
\[log(p_k(\x) / p_1(\x)) = b_k(\boldsymbol{x)} \]

\[ b_k(\boldsymbol{x)} \sim Logistic(\mu_k + \boldsymbol{b}^{(np)T}_k \x, \sigma) \quad (k=2,...,K), \quad b_1 = 0 \]
