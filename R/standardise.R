...
#' covariate value.
#'
#' @details These are produced by generating a Monte Carlo
#' sample from the joint distribution of parameters \eqn{\theta} and
#' covariate values \eqn{X}, \eqn{p(X,\theta) = p(\theta|X)p(X)}, where \eqn{p(X)}
#' is defined by the empirical distribution of covariates in the
#' standard population.
#'
...
#' standardised samples should be obtained by concatenating the posterior
#' samples for each covariate value in the standard population.
#' The sample from the standardised posterior of parameters then has
#' size \code{niter} times the number of rows in \code{newdata},
#' where \code{niter} is the number of MCMC iterations used in the
#' original \code{survextrap} fit.
#' Computing the resulting output function (e.g. RMST which uses
#' numerical integration) can then be computationally intensive if this
#' sample size is large.
#'
#' A quicker alternative is to sample a random
#' row of the standard population for each MCMC iteration.  The
#' standardised sample from the posterior then has size \code{niter}.
#' This is specified by using \code{random=TRUE}.  If this is used,
#' then the result depends on the random number seed, and it should be
#' checked that the results are stable to within the required number
#' of significant figures.  If not, run \code{survextrap} with more MCMC iterations
#' or increase \code{nstd} here.
#'
#' @param nstd Number of draws from the population distribution used
#'   per MCMC sample from the parameters when \code{random=TRUE}.  With the default of 1, the
#'   value of the covariate vector \eqn{X} is essentially treated as if it
#'   were an additional parameter in the Bayesian model, drawn by
#'   Monte Carlo independently of the remaining parameters.
#'
...
#' g(\theta|X) dX}.
#'
#' The distribution of \eqn{X} is defined by the empirical distribution of
#' values in the \code{newdata} supplied to \code{get_pars}.
#'
